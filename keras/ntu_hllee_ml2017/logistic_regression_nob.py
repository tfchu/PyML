...
def predict(weights, features):
    return sigmoid(weights.dot(features.T))

# weights: 1 x i
# features: n x i
# lables: 1 x n
def update_weights_bias(weights, lr, w_lr):
    # 1 x n
    predictions_t = predict(weights, features_t)
...
def train(weights, lr, w_lr, iters):
    cost_history = []

    for i in range(iters):
        weights, w_lr = update_weights_bias(weights, lr, w_lr)
        #print(weights)
        #print(weights, bias)
...
def accuracy(weights):
    predictions = predict(weights, features_v)
    #print(predictions)
...
def plot_contour(weights):
    ...
    for i in range(len(y)):
        for j in range(len(x)):
            ...
            Z[i][j] = predict(weights, np.array([[x[j], y[i]]]))
    #print(Z[10])
    fig, ax = plt.subplots()
...
def main():
    ...
    w_lr = 1.
    iteration = 100000
    weights = train(w0, lr, w_lr, iteration)
    print(weights)
